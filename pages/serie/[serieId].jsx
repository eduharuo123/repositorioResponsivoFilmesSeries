...
export default function SerieDetalhes(){
    ...
    useEffect(() => { 
        const Id = router.query.serieId     
        if(!router.isReady) return;

        fetch(`${urlBase}tv/${Id}?api_key=${key}&language=pt-BR`)
        .then(response => response.json())
        .then(response => {
            
            setDetalhes(response)

            fetch(`${urlBase}tv/${Id}/videos?api_key=${key}`)
            .then(response => response.json())
            .then(videoData => {
                console.log(videoData)
                const tr = videoData.results.find(vid => vid.name = 'Oficial Trailer')
                setTrailers(tr)

                fetch(`${urlBase}tv/${Id}/credits?api_key=${key}`)
                .then(response => response.json())
                .then(response => {
                ...
                <div className={styles.conteudoContainer}>
                    ...
                    <div className={styles.elencoContainer}>
                        ...
                        <div className={styles.atores}>
                            {   
                                 elenco.cast && elenco.cast.map((ator, i) => {
                                    if(ator.profile_path == null){
                                        return <ElencoCardV2 key={i} 
